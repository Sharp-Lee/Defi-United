...
| P10d | Clean architecture rebase spec | `main` | pending | in progress | `git diff --check` pending | no | no | Spec aligns the professional shell and future 985monitor-class product lanes before implementation planning. |
| P11-P18 | Full wallet workbench product target | `main` | pending | in progress | `git diff --check` pending | no | no | Product capability spec records account, asset, queue, distribution, inscription, ABI, reverse parsing, and mobile/release requirements. |
