...
| P10c | Chain / RPC config and shared fee panel plus vault import hardening | `main` | local P10c commit on top of `1a14300` | passed | `npm test`; `npm run typecheck`; `npm run build`; `npm run smoke:browser`; `git diff --check` | no | yes | Browser-side chain/RPC config, shared fee panel, password-verified vault import, stronger KDF policy, production/mobile smoke, and PWA icons are locally verified. |
