...
| P10a | PWA architecture baseline and Chinese shell | `codex/p10-pwa-wallet-spec` | `pending` | passed | passed | no | no | 默认入口切换为 browser-first PWA shell，新增中文一级导航、页面骨架、响应式布局和 PWA manifest/installability baseline；自动测试、typecheck、build、按需 Playwright Chromium smoke 和 whitespace 检查通过；不实现 vault、签名、广播、RPC 提交或真实 history 写入。 |
