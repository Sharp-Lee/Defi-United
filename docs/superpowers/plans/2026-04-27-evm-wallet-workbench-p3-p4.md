...
> 面向 subagent-driven-development：本文件同时保留 P3、P4-1 到 P4-13、P5-1 到 P5-4 的完成记录，以及 P6 backlog。只有标记为未完成、探索或后续计划的条目表示待执行任务，不代表能力已经完成；后续任务仍由 controller 按任务串行派发和收口。
...
- 更新必要的 spec/plan 状态说明：P4-8 到 P4-13 已完成；当前主线已进一步完成 P5-1 到 P5-4，P6 仍为计划/后续能力。
...
- 诊断输出和导出不包含敏感材料。
- 文档状态准确：P4-1 到 P4-13 按实际完成情况标记；当前主线已进一步完成 P5-1 到 P5-4，P6 仍为计划/后续能力。
...
##### Task P5-4a: asset/allowance/revoke spec/design（状态：已完成）
...
- 不把 P6 写成当前已完成能力；README 可把 P5-4 已合入能力更新为当前可用，但不能承诺 full portfolio、NFT collection discovery、全量授权发现或 batch revoke。
...
- 验证命令：`git diff --check`，并用 `rg` 检查 P5-4 状态措辞与 P6 future boundary。
...
##### Task P5-4d: desktop asset/approval scanner UI and filters（状态：已完成）
...
##### Task P5-4e: revoke draft/frozen key/confirmation UI（状态：已完成）
...
- 已完成实现保留 submit disabled/stub，未签名、未广播、未写 history；P5-4f 继续承接 Rust submit、submit-time revalidation、history/recovery。
- 建议验证：`npm test -- src/features src/core`、`npm run typecheck`、`git diff --check`。

##### Task P5-4f: revoke submit Rust command + history/recovery（状态：已完成）
...
##### Task P5-4g: integration/security regressions（状态：已完成）
...
P6-1 在 Tauri desktop 主线中提供只读的 tx hash 逆向解析入口。用户输入 tx hash 并选择/验证 chain/RPC 后，应用读取 transaction、receipt、logs、block/code 和相关 local/ABI/metadata context，展示可解释的 interaction summary。输出是 analysis/read model，不是交易 truth source，不签名、不广播、不自动恢复或修改 history。

##### Task P6-1a: tx hash reverse parsing spec/design（状态：已完成，doc-only）

**目标**

补齐项目级 P6-1 spec/design，并把 tx hash 逆向解析拆成可串行执行的后续任务。明确 product workflow、data source trust model、analysis read model、privacy/diagnostics、storage/history integration、UX boundary 和验收方向。

**依赖**

- P5-1/P5-2 ABI source/cache/selector/decode 能力。
- P5-3 raw calldata 的 selector/length/hash/bounded preview 约束。
- P5-4 asset approval revoke typed history metadata。
- P4-2 diagnostics/export redaction 和 existing history recovery semantics。
...
- Spec 覆盖 tx hash + selected/validated chain/RPC 输入、可选 ABI/explorer metadata context，以及 output 只是 explanatory analysis/read model。
- Spec 明确 RPC、local history、ABI registry/cache、explorer/indexer metadata 的 trust model，并要求 unknown/proxy/unverified/conflict 状态显式显示。
- Spec 明确 transaction identity、classification candidates、receipt/log summary、ABI decode、risk/uncertainty statuses。
- Spec 明确 full calldata/log payload、RPC URL/API key/raw signed tx/secrets 的 redaction/bounding 边界。
- Spec 明确 network privacy boundary：RPC/explorer/indexer lookups reveal interest in a tx/address; outbound queries are minimal and must not upload local history matches、account labels、notes、address book labels、wallet inventory 或 broad local context；UI keeps provider/source visibility clear.
...
实现只读 Rust/Tauri command：校验 tx hash 与 expected chain/RPC，读取 transaction、receipt、receipt logs、block/base fee/timestamp 和 address code，并返回 bounded fetch read model。

**依赖**

- P6-1a spec/design。
- 现有 RPC chainId validation、provider config redaction、command error model。

**边界/非目标**

- 不做 ABI/function/event semantic decode。
- 不写 transaction history、不保存 analysis、不签名、不广播。
- Explorer/indexer metadata 可保持 absent/stub，不影响 RPC facts 展示。

**验收/测试建议**

- Rust tests 覆盖 valid confirmed tx、missing tx、pending/no receipt、reverted receipt、wrong chain/RPC、block unavailable、code unavailable、contract creation、minimal outbound request shape 和 provider failure redaction。
...
基于 P6-1b fetch model 与 P5-1/P5-2 ABI cache，生成 function/event/error decode candidates、classification candidates、confidence/source summaries 和 uncertainty statuses。

**依赖**

- P6-1a spec/design。
- P6-1b RPC fetch model。
- P5-1/P5-2 ABI registry/cache/selector summary。

**边界/非目标**

- 不自动 fetch arbitrary ABI，不把 explorer/indexer verified label 当 truth。
- 不做风险评分库、合约安全审计或 hot contract trend 分析。
- 不用 selector/event match 覆盖链上 identity。

**验收/测试建议**

- Unit tests 覆盖 ERC-20 transfer/approval/revoke candidates、batch disperse、managed ABI call、raw calldata unknown selector、contract creation、selector collision、overloaded function、stale ABI、unverified ABI、malformed calldata 和 event topic conflict。
...
实现 Tauri desktop tx hash analysis UI：输入 tx hash、选择 expected chain/RPC、展示 fetch/decode/local history sections、unknown/conflict badges、copyable summaries 和 read-only action boundaries。

**依赖**

- P6-1a spec/design。
- P6-1b Rust read-only command。
- P6-1c analysis read model。
- Existing AppShell/history/diagnostics UI patterns。

**边界/非目标**

- 不提供 submit、rebroadcast、replace/cancel、revoke 或 automatic recovery action。
- 不把 explorer/indexer label、token symbol/name 或 ABI decode 写成确定事实。
- UI must make provider/source visibility clear, so users can see when a lookup or enrichment came from RPC, local history, ABI cache, explorer or indexer.
- 不复制 full sensitive URLs/secrets 或 unbounded calldata/log payload。

**验收/测试建议**

- Frontend tests 覆盖 tx hash validation、chain/RPC mismatch display、missing tx、pending/no receipt、reverted, decoded ERC-20/ABI, unknown selector、selector conflict、missing logs、copy buttons、provider/source labeling 和 local history no-match/match states。
...
把 local transaction history typed metadata 与 reverse analysis side-by-side 展示，并补 diagnostics/export/history redaction 回归，确保 analysis troubleshooting 不泄漏 secrets 或 unbounded payload。
...
- P6-1b fetch model。
- P6-1c analysis read model。
- Existing native/ERC-20/batch/ABI/raw calldata/asset approval revoke history schemas。

**边界/非目标**

- Reverse analysis 默认不 mutate history，不自动 attach note，不自动 recover broadcasted records。
- 不把 local history metadata 覆盖链上 facts。
- 不新增 signing/broadcast/recovery side effects。

**验收/测试建议**

- Tests 覆盖 local native/ERC-20/batch/ABI/raw calldata/asset approval revoke history match、chain/local conflict、nonce/from mismatch、no-match 和 duplicate tx hash handling。
- Redaction tests 覆盖 RPC URL secret、API key、query token、private key、mnemonic、raw signed tx、full calldata、full logs、large revert data 和 provider error truncation。
...
串联 RPC fetch、ABI/event decode、frontend UI、local history match 和 diagnostics/export，形成 P6-1 跨层回归，并确认既有交易/扫描/revoke 能力不回退。

**依赖**

- P6-1b Rust read-only command。
- P6-1c decode/read model。
- P6-1d frontend UI。
- P6-1e local history/diagnostics integration。

**边界/非目标**

- 不扩大为 contract address hot analysis、indexer product、portfolio/NFT discovery、batch revoke、risk scoring 或 wallet recovery automation。
- 不引入真实 production API key、真实 private key/mnemonic 或 secret-bearing fixtures。

**验收/测试建议**

- Integration tests 覆盖 confirmed ERC-20 transfer、ERC-20 approval/revoke、managed ABI write、raw calldata unknown selector、native transfer、batch disperse、contract creation、pending/no receipt、reverted receipt、proxy uncertainty、wrong chain/RPC 和 RPC unavailable。
- Security regressions 覆盖 no full calldata/log persistence、secret redaction、minimal outbound provider queries、selector collision uncertainty、unverified/stale ABI wording 和 explorer/indexer advisory labeling。
- 建议验证：`npm test -- src/features src/core`、`npm run typecheck`、`cargo test --manifest-path src-tauri/Cargo.toml`、`scripts/run-anvil-check.sh`、`git diff --check`。
...
- 不把浏览器版重新设为后续主线。
- 不把 P6 计划能力写成当前已完成，也不把已完成的 P4/P5-1/P5-2/P5-3/P5-4 历史任务重新写成待交付。
