...
- 当前 v1 runtime 已提供受控 raw calldata sender/preview、P5-4 资产/授权扫描与 revoke workflow，以及只读 tx hash 逆向解析入口，但不作为通用链测试平台、安全解析器、full portfolio/NFT discovery、batch revoke 或 hot contract trend 工具。
...
Hot contract 分析、复杂资产组合展示、full portfolio/NFT collection discovery、全量授权发现和 batch revoke 仍属于后续/计划能力，不能列入当前能力。
...
当前真实可用交易/调用类型包括 native transfer、ERC-20 transfer、native batch、ERC-20 batch、managed ABI read/write caller、raw calldata sender/preview、asset approval revoke 和只读 tx hash 逆向解析。资产/授权扫描作为只读 read model 能力已可用，但不承诺全量发现。
Hot contract 分析仍未完成。新增交易能力必须继续使用显式 typed Intent/Submission 契约，不能把不同交易类型塞进 native transfer 字段里伪装成已支持；新增只读 analysis 必须保持 advisory/read model 边界，不得伪装成可提交交易。
...
### 9.11 Contract address hot 交易/selector 分析入口（P6-2 计划/设计）

P6-2 目标是在 Tauri desktop 主线中提供按合约地址的只读 hot interaction analysis：用户输入合约地址，或从 P6-1 tx hash 解析结果中的 `to` / created contract 跳转，选择 chain/RPC 与已配置 explorer/indexer sampling source 后，应用读取一组受限样本交易、ABI/cache metadata 和 selector/topic 摘要，展示热门 selector、常见交互模式、样本边界和不确定性。该入口是 advisory/read model，不是风险评分器、审计工具、交易推荐器或全量链上索引器。
...
- Contract address hot 交易/selector 分析：入口支持直接输入合约地址，或从 tx hash 逆向解析结果跳转到合约地址。contract address 入口依赖 explorer/indexer、ABI/selector 数据库或采样交易数据，用于分析热门 selector、交互模式和风险提示；它存在数据源缺失、采样偏差、代理合约、未验证合约、selector 冲突和链特异性边界，不能把推断结果展示为确定事实。
