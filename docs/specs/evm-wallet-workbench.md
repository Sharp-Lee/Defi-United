...
v1 已有基础历史记录和状态字段。更完整的三层展示、状态解释、状态迁移细节和恢复入口属于后续 P3/P4 hardening 范围。

## 10. 后续 P3/P4 方向

### 10.1 History UX hardening

后续/计划：

- 将历史记录从单列表升级为更清晰的交易详情视图。
- 明确展示 Intent、Submission、ChainOutcome 三层信息。
- 展示状态迁移时间线、错误原因、RPC 返回摘要和用户可执行恢复动作。
- 为 pending、replaced、cancelled、dropped 提供更明确的解释和视觉区分。
- 对长时间 pending 的交易提供更可靠的 replace/cancel 引导。

### 10.2 交易状态三层展示

后续/计划：

- 在 UI 中分别展示用户输入、最终提交参数和链上结果，避免只显示最终 hash。
- 支持按 `account + chain + nonce` 聚合 replacement 关系。
- 将普通转账、replace、cancel 关联成一组可审计记录。
...
- 对 dropped 判定提供复核和重新 reconcile 能力。
