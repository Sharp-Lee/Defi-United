...
- P4-1 已提供本地结构化日志基线；P4-2 的诊断面板/导出仍属于后续计划能力。

### 8.8 恢复与补录边界

- 历史文件不可读或疑似损坏时，提交新交易前必须停止并给出明确恢复路径，不能为了保持 UI 可用而绕过本地历史读取。
- 损坏恢复应优先隔离原文件、保留可审计副本、生成用户可理解的错误摘要，再允许用户选择修复或重建索引。
- 广播成功但历史写入失败时，系统必须把 tx hash、chainId、account/from、nonce 和写入错误返回给用户；后续补录入口在 P4-4 中实现，当前不能假装已经自动补齐。
...
### 10.3 P4-1 诊断事件与本地结构化日志已完成

P4-1 已在当前分支提交，作为 P4 后续诊断/恢复任务的基线能力。当前完成范围是本地非敏感结构化诊断事件，不等同于完整诊断 UI 或恢复工具。
...
- pending 老化策略和自动提示。
- anvil smoke check 的完整诊断增强与 P4 回归。
...
- 基于已完成的 P4-1 本地结构化日志，继续补齐用户可见诊断入口。
