...
- 当前 v1 runtime 已提供受控 raw calldata sender/preview 和 P5-4 资产/授权扫描与 revoke workflow，但不作为通用链测试平台、安全解析器、full portfolio/NFT discovery 或 batch revoke 工具；tx hash 逆向解析和 hot contract 分析仍是后续计划。
...
Tx hash 逆向解析、hot contract 分析、复杂资产组合展示、full portfolio/NFT collection discovery、全量授权发现和 batch revoke 仍属于后续/计划能力，不能列入当前能力。
...
当前真实可用交易/调用类型包括 native transfer、ERC-20 transfer、native batch、ERC-20 batch、managed ABI read/write caller、raw calldata sender/preview 和 asset approval revoke。资产/授权扫描作为只读 read model 能力已可用，但不承诺全量发现。
Tx hash 逆向解析和 hot contract 分析仍未完成。新增能力必须继续使用显式 typed Intent/Submission 契约，不能把不同交易类型塞进 native transfer 字段里伪装成已支持。
