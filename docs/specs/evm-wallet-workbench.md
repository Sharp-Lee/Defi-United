...
- P5-4 资产/授权扫描与 revoke：提供受 source coverage 约束的 ERC-20 balance/allowance、已知 NFT approval point 扫描视图，并对明确 active approval 提供受控 revoke workflow；不承诺 full portfolio、全量 NFT collection discovery、全量授权发现或 batch revoke。
...
当前真实可用交易/调用类型包括 native transfer、ERC-20 transfer、native batch、ERC-20 batch、managed ABI read/write caller、raw calldata sender/preview、asset approval revoke、只读 tx hash 逆向解析和只读 hot contract analysis。资产/授权扫描作为只读 read model 能力已可用，但不承诺全量发现；hot contract analysis 作为 advisory/read model 能力已可用，但不承诺 risk scoring、审计结论、全链 trend 或全量索引。
新增交易能力必须继续使用显式 typed Intent/Submission 契约，不能把不同交易类型塞进 native transfer 字段里伪装成已支持；新增只读 analysis 必须保持 advisory/read model 边界，不得伪装成可提交交易。
...
- `assetApprovalRevoke`：当前真实可用的受控 approval revoke intent/submission 能力。
- 后续可再扩展 `txHashAnalysis` 等只读 analysis/note 类型；资产扫描本身是 read model/snapshot 能力，不应伪装成链上交易。
...
### 9.9 资产/授权扫描与 revoke 工作流（P5-4 已完成设计/能力边界）
...
- Doc-only 验证命令：`git diff --check`，并用 `rg` 检查 README 与本 spec 中的 P5-4 状态措辞，同时确认 P6 没被写成已完成能力。

### 9.10 Tx hash 逆向解析入口（P6-1 计划/设计）

P6-1 目标是在 Tauri desktop 主线中提供按交易 hash 的只读逆向解析入口：用户输入 tx hash，并选择或验证目标 chain/RPC 后，应用读取链上 transaction、receipt、logs 和可用 metadata，生成可解释的交互摘要。该结果是 analysis/read model，用于帮助用户理解一笔已发生或待确认交易，不是交易真相源、不是签名/广播流程，也不自动修复或恢复历史。

**Product workflow**

- 输入至少包含 `tx_hash`、expected `chainId` 和 selected RPC profile。执行前必须 probe RPC `chainId`，actual 与 expected 不一致时阻塞并显示 wrong chain/RPC。
- 用户可以选择本地 ABI registry/cache、已选 ABI source 或后续配置的 explorer/indexer metadata 作为辅助上下文；没有上下文时仍应展示链上 identity 与 unknown decode 状态。
- 输出是 read-only analysis：transaction identity、receipt/log 摘要、可能的分类候选、ABI/event decode、local history match 和风险/不确定性。入口不得创建签名 draft、不得提交交易、不得隐式 recover history。

**Data sources and trust model**

- RPC 是链事实的主要来源：`eth_getTransactionByHash`、`eth_getTransactionReceipt`、receipt logs、block/base fee/timestamp（如可用）、`eth_getCode` 或等价 code-at-address probing。
- Local history 可以按 tx hash、`chainId`、from、nonce 或 typed metadata 匹配本地提交；匹配结果应与链上 facts 并排展示，不能覆盖链上 identity。
- P5-1/P5-2 ABI registry/cache 可用于 decode 已知 function selectors、events 和 errors。Decode 只有在 ABI source selected/valid 且 selector/topic 匹配时才显示对应 confidence/source/version/hash。
- Explorer/indexer metadata 后续可作为 label、verified ABI、proxy hint、token metadata 或 candidate source，但必须标记为 untrusted/advisory。Explorer label、verified badge、token symbol/name 或 indexer result 不能覆盖 `chainId + address + tx hash` identity。
- Unknown、proxy/implementation uncertainty、unverified contract、selector collision、stale ABI source、malformed calldata、event decode conflict 和 missing logs 都必须保留为显式状态。

**Analysis read model**

- Transaction identity 至少包含 hash、expected/actual `chainId`、from、to（或 contract creation）、nonce、native value、input selector、calldata byte length、`calldata_hash_version = keccak256-v1`、calldata hash、block number/hash/timestamp、status/pending state 和 RPC identity summary。
- Classification candidates 可以包括 native transfer、ERC-20 transfer、ERC-20 approval/revoke、batch disperse、managed ABI call、raw calldata、asset approval revoke、contract creation 和 unknown contract call。多候选或低置信度时展示 candidates/confidence，不使用确定语气。
- Receipt/log summary 至少包含 receipt status、gas used、effective gas price、logs count、known `Transfer`/`Approval` event summaries、decoded events（当 ABI 可用）和 missing/no receipt 状态。缺失 logs 不能推断为没有资产变化。
- ABI decode 至少包含 selector match count、selected ABI source/version/hash/fingerprint、function signature、argument summaries、decode confidence、source stale/conflict 状态和 decode error。参数展示必须是 bounded summary，不保存无边界 raw payload。
- Risk/uncertainty statuses 至少覆盖 missing tx、pending/no receipt、reverted/failed receipt、wrong chain/RPC、proxy/implementation uncertainty、selector collision、unverified ABI、stale ABI/cache、malformed calldata、event decode conflict、missing logs、RPC/indexer unavailable 和 contract creation unknown init code。

**Privacy and diagnostics**

- Network privacy boundary: RPC/explorer/indexer lookups reveal interest in a tx hash, address, contract, selector/topic or chain. Outbound P6-1 queries must be minimal and purpose-bound: send only the tx hash, chain/RPC request, address/code/block/log lookup fields needed for the selected analysis source, and never upload local history matches, account labels, user notes, local address book labels, wallet inventory, or broad local context to providers.
- UI must keep provider/source visibility clear before and after lookups, including whether data came from RPC, local history, ABI cache, explorer or indexer. Explorer/indexer enrichment should be opt-in or clearly source-labeled when configured, so users understand which provider learns about the requested tx/address.
- Diagnostics/export/history 不得持久化无边界 full calldata、full log payload、raw signed tx、private key、mnemonic、seed、明文密码、RPC secret、API key、query token 或完整认证 URL。
- Calldata、logs、revert data 和 provider errors 只允许 bounded preview、length、hash、selector/topic、truncation metadata 和脱敏错误摘要。大 payload 必须按上限截断。
- Diagnostic events 只用于 troubleshooting，不能被 UI 当作 analysis truth。诊断中可以记录 tx hash、chainId、address/selector/topic 摘要、error class 和 source kind，但不能记录 secret-bearing URL。

**Storage / history integration**

- Reverse analysis 默认不修改 transaction history、token/approval snapshots 或 ABI cache selection。它可以读取 local history match，但不得自动写入、重建或覆盖 history record。
- 如果 tx hash 匹配本地 history，UI 展示 local typed metadata（native/ERC-20/batch/ABI/raw calldata/asset approval revoke 等）与 chain facts 的一致/冲突状态；本地 metadata 不得覆盖链上 from/to/value/status。
- Future “save analysis”、“attach note” 或 “recover into history” 必须是显式、非破坏性操作，并继续复用既有 history recovery 边界。任何保存都不得引入无边界 calldata/log payload。
- 入口无签名、无广播、无 recovery side effect，也不得触发 revoke、replace/cancel 或 rebroadcast。

**UX / UI boundaries**

- P6-1 是 desktop/Tauri feature；React 负责输入、read model 展示和 copy actions，Rust/Tauri 负责 RPC 读取、chainId 校验、可复用脱敏和本地数据访问。
- Unknown、ambiguous、advisory、stale 和 conflict 状态必须可见；对推断分类使用 “candidate/possibly/unknown” 语义，不把 decode 或 explorer label 写成确定事实。
- Copy affordances 应覆盖 tx hash、address、selector、topic、calldata hash 和 ABI source hash 等摘要；不得提供 full sensitive URL、API key、raw signed tx 或无边界 payload 的复制入口。
- Explorer/indexer labels、token names、icons 和 verified badges 只能作为附加 metadata，不能替代链、地址或合约身份。

**任务拆分 / 验收方向**

- P6-1a spec/design：补齐本节和 SDD plan，doc-only。验收：覆盖 workflow、data trust、analysis model、privacy、history integration、UX boundary 和后续任务拆分；验证 `git diff --check`。
- P6-1b Rust read-only command + fetch model：实现 tx hash 输入校验、RPC chainId probe、transaction/receipt/log/block/code 读取和 bounded error model，并保证 outbound RPC/provider requests 不携带 local history match、account labels、notes 或 broad local context。非目标：不 decode ABI、不写 history、不签名广播。测试覆盖 missing tx、pending receipt、wrong chain、reverted receipt、block unavailable、code unavailable、minimal outbound request shape 和 redaction。
- P6-1c ABI/event selector decode engine/read model：消费 P5-1/P5-2 ABI cache，输出 function/event/error decode candidates、source identity、confidence、conflict/stale/malformed 状态。非目标：不 fetch arbitrary ABI、不做 risk scoring。测试覆盖 selector collision、overloaded signatures、stale source、unverified/no ABI、event topic conflict 和 malformed calldata/log。
- P6-1d frontend UI for tx hash analysis：提供桌面入口、chain/RPC validation 状态、analysis sections、copyable summaries、unknown/conflict badges、provider/source visibility 和 local history match display。非目标：不提交交易、不自动保存 analysis。测试覆盖输入校验、pending/no receipt、decoded/unknown states、copy actions、provider/source labeling 和 responsive layout。
- P6-1e local history/diagnostics integration + redaction regressions：把 local typed metadata side-by-side 接入 read model，并补 diagnostics/export redaction。非目标：不自动 mutate history。测试覆盖 native/ERC-20/batch/ABI/raw calldata/asset approval revoke history match、chain/local conflict、no-match、full calldata/log exclusion 和 secret redaction。
- P6-1f integration/security regressions：串联 RPC fetch、decode、UI、history match 和 diagnostics，确认现有 native/ERC-20/batch/ABI/raw calldata/asset approval revoke 功能未回退。测试覆盖 happy path decoded ERC-20 transfer/approval、managed ABI call、raw calldata unknown selector、contract creation、proxy uncertainty、indexer advisory label、wrong chain 和 unavailable RPC。

### 9.11 Contract address hot 交易/selector 分析入口（P6-2 已完成设计记录）

P6-2 已在 Tauri desktop 主线中提供按合约地址的只读 hot interaction analysis：用户输入合约地址，或从 P6-1 tx hash 解析结果中的 `to` / created contract 跳转，选择 chain/RPC 与已配置 explorer/indexer sampling source 后，应用读取一组受限样本交易、ABI/cache metadata 和 selector/topic 摘要，展示热门 selector、常见交互模式、样本边界和不确定性。该入口是 advisory/read model，不是风险评分器、审计工具、交易推荐器或全量链上索引器。

**Product workflow**

- 输入至少包含 `contract_address`、expected `chainId`、selected RPC profile 和可选 sampling source。执行前必须 probe RPC `chainId`，actual 与 expected 不一致时阻塞并显示 wrong chain/RPC。
- 入口支持两种 seed：用户直接输入合约地址；或从 P6-1 tx hash analysis 中选择 `Analyze contract`，把 tx 的 `to` 或 receipt `contractAddress` 作为合约地址。tx hash seed 只能带入链上合约身份，不能上传 local history match、账户标签、notes 或 wallet inventory。
- RPC-only 模式只能展示合约 code presence、code hash、ABI/cache availability 和本地 selector metadata；它不能声称有 hotness、交易频率或近期 trend。需要 hot samples 时必须选择已配置 explorer/indexer/data source，并在 UI 上明确 source learns the contract address。
- 用户可以设置 bounded sampling window，例如 latest N samples、block range 或 provider-supported cursor，但初版必须设置硬上限和默认上限，避免全量抓取、无限分页或后台监控。
- 输出是 read-only analysis：contract identity、source/sample coverage、selector/topic frequency candidates、decoded function/event candidates、example tx hashes 和 uncertainty statuses。入口不得创建签名 draft、不得提交交易、不得隐式保存 analysis 或修改 history/cache。

**Data sources and trust model**

- RPC 是 chain identity 与 code facts 的主要来源：`eth_chainId`、`eth_getCode`、可选 latest block/base fee/timestamp 和 P6-1 可复用的 tx/receipt lookup。RPC 不能提供完整历史热度；RPC-only 结果必须显示为 coverage-limited。
- Explorer/indexer sampling source 是 untrusted/advisory source：可以提供 transaction sample、method id、timestamp、value、from/to、status、gas、logs 或 token transfer hints，但不能覆盖 `chainId + contractAddress + txHash` 身份。
- P5-1/P5-2 ABI registry/cache 可用于 decode selectors、events 和 errors。Decode 只有在 ABI source selected/valid 且 selector/topic 匹配时才显示对应 source/version/hash/fingerprint/confidence。
- Selector/topic databases、4byte-like labels、explorer method labels、verified badges、proxy hints 和 token labels 均为 advisory metadata，必须标注 source、confidence 与 conflict/stale/unverified 状态。
- Unknown、proxy/implementation uncertainty、unverified contract、selector collision、event topic conflict、stale ABI/cache、provider sampling unavailable、partial sample、rate limited、malformed provider payload、missing logs 和 mixed direction 必须保留为显式状态。

**Hot analysis read model**

- Contract identity 至少包含 expected/actual `chainId`、contract address、code status、code byte length、`code_hash_version = keccak256-v1`、code hash、selected RPC identity summary 和 provider/source visibility。
- Sample coverage 至少包含 source kind/id、query window、sample count、omitted/limited count、oldest/newest block/time（如可用）、provider status、rate-limit status 和 whether sample is complete/partial/unknown。
- Selector summary 至少包含 selector、call count、share within sample、unique sender count（如 provider 支持）、native value aggregate summary、success/revert/unknown count、first/last seen block/time、decoded signature candidates、source/confidence 和 example tx hashes。所有计数都必须以 “sampled” 或 “within selected source” 语义展示。
- Interaction pattern candidates 可以包括 ERC-20 transfer/approval/revoke、batch disperse、managed ABI call、raw/unknown call、contract creation interaction、proxy admin-like selector candidate 和 event-heavy interaction candidate。多候选或低置信度时展示 candidates/confidence，不使用确定语气。
- Event/topic summary 至少包含 topic0、sample log count、decoded event candidates、source/confidence、example tx hashes 和 conflict/malformed 状态。缺失 logs 不能推断为没有事件或没有资产变化。
- Example rows 只能保存/展示 bounded summary：tx hash、block number/time、from/to/contract relation、selector/topic0、value wei、status label、calldata/log byte length 与 hash。不得展示 full calldata、full logs、full revert data 或 provider raw payload。

**Privacy and diagnostics**

- Network privacy boundary: explorer/indexer lookups reveal interest in a contract address, selector/topic, block range or sample cursor. Outbound requests must be minimal and purpose-bound: only send chain/source identity, contract address and bounded window/cursor fields required by the chosen source.
- P6-2 must never upload local history matches, account labels, user notes, address book labels, wallet inventory, token watchlist inventory, full local ABI catalog or broad local context to external providers.
- API keys, query tokens, RPC secrets, full provider URLs and auth headers must stay in Rust/Tauri secret/config boundaries. React state, diagnostics, export, history and test snapshots may only contain redacted endpoint summaries/fingerprints and source labels.
- Diagnostics/export/history 不得持久化 full calldata、full log payload、full provider response body、raw signed tx、private key、mnemonic、seed、明文密码、RPC secret、API key、query token 或完整认证 URL。
- Diagnostic events can record contract address, chainId, source kind, selector/topic, bounded sample count, status and sanitized error class for troubleshooting, but cannot become analysis truth.

**Storage / history integration**

- Hot analysis 默认不写 transaction history、token/approval snapshots、ABI cache selection 或 provider cache。它读取 ABI registry/cache 和 optional source config，但不得自动修改它们。
- Future “save analysis”、“watch contract”、“attach note” 或 “import ABI from provider” 必须是显式、非破坏性操作，并继续复用 ABI/cache validation、secret redaction 和 bounded payload rules。
- 如果样本 tx hash 与本地 history 有匹配，初版只允许显示 count/has-local-example 之类的 bounded local hint；不得把 local record details 上传给 provider，也不得用 local history 覆盖 provider/RPC facts。
- 入口无签名、无广播、无 recovery side effect，也不得触发 revoke、replace/cancel、rebroadcast 或 batch action。

**UX / UI boundaries**

- P6-2 是 desktop/Tauri feature；React 负责输入、source visibility、read model 展示和 copy actions，Rust/Tauri 负责 RPC/source reads、chainId 校验、source adapter、脱敏和 bounded model。
- Unknown、ambiguous、advisory、partial、stale 和 conflict 状态必须可见；热门/常见 selector 使用 “sampled/candidate/advisory” 语义，不写成链上全局事实。
- Copy affordances 应覆盖 contract address、selector、topic、code hash、ABI source hash、sample tx hash 和 source fingerprint 等摘要；不得提供 full sensitive URL、API key、raw provider payload、full calldata 或 full logs 的复制入口。
- Explorer/indexer labels、verified badges、token names、icons 和 method labels 只能作为附加 metadata，不能替代 chain、contract address、selector/topic 或 tx hash identity。

**任务拆分 / 验收方向**

- P6-2a spec/design + implementation plan：补齐本节和 P6-2 SDD plan，doc-only。验收：覆盖 workflow、data trust、read model、privacy、storage/history integration、UX boundary 和后续任务拆分；验证 `git diff --check`。
- P6-2b Rust source config/read-only fetch model：实现 contract address 输入校验、RPC chainId probe、code summary、configured sampling source validation、bounded sample fetch adapter 和 provider error model。非目标：不 decode ABI、不写 history、不签名广播、不真实依赖 production API key。测试覆盖 invalid address、wrong chain、RPC unavailable、code absent、source missing/rate-limited/malformed、bounded sample cap、minimal outbound request shape 和 secret redaction。
- P6-2c selector/topic aggregation + ABI decode read model：消费 sample fetch model 与 P5-1/P5-2 ABI cache，输出 selector/topic frequency candidates、decoded signature/event candidates、coverage summary 和 uncertainty statuses。非目标：不做 risk scoring、不做全链 trend、不自动 import ABI。测试覆盖 selector collision、event topic conflict、stale/unverified ABI、proxy uncertainty、unknown selector、partial samples、missing logs、malformed calldata/log 和 bounded value caps。
- P6-2d frontend UI for hot contract analysis：提供 desktop 入口、contract/tx-seed 输入、chain/RPC/source validation 状态、sample coverage、selector table、event/topic summary、example rows、copy actions、advisory/partial/conflict badges 和 provider visibility。非目标：不自动保存 analysis、不触发交易动作。测试覆盖输入校验、RPC-only limited state、sampled selector rows、unknown/conflict/stale states、copy actions、source labels 和 responsive layout。
- P6-2e diagnostics/privacy/local integration：补 diagnostics/export redaction、source disclosure copy 和 optional local history bounded hint。非目标：不展示 local history record details、不上传 local context。测试覆盖 provider secret redaction、full calldata/log/provider body exclusion、local hint bounded display、diagnostic scope redaction 和 source visibility。
- P6-2f integration/security regressions：串联 source fetch、aggregation/decode、UI、diagnostics 和现有 P6-1/P5 workflows，确认 native/ERC-20/batch/ABI/raw calldata/revoke/tx hash analysis 未回退。测试覆盖 sampled ERC-20 transfer/approval selectors、managed ABI selector, unknown raw selector、proxy uncertainty、wrong chain、source unavailable、rate limit、partial sample、no full payload persistence 和 anvil smoke baseline。
...
P4-1 到 P4-13 已在主线完成，作为后续 P5/P6 探索任务的诊断、恢复和交易能力基线。当前完成范围包括原生币转账、ERC-20 转账、token watchlist/balance scanning、多账户编排、native/ERC-20 batch、历史恢复、诊断导出、dropped 复核、pending 老化和 anvil smoke 回归，不等同于通用链测试平台、raw calldata 钱包、资产授权扫描工具、risk scoring 工具或 full-chain hot contract trend 工具。
...
- 资产/授权扫描与 revoke：已提供受 source coverage 约束的 ERC-20 balance/allowance 和已知 NFT approval point 扫描视图，并对明确 active approval 提供受控 revoke 工作流；不承诺 full portfolio、全量 NFT collection discovery、全量授权发现或 batch revoke。
- Tx hash 逆向解析：已提供只读入口，按交易 hash 获取 transaction、receipt、logs、block/code、ABI decode 和 local history side-by-side 摘要；它不是交易 truth source，不签名、不广播、不自动恢复或修改 history。
- Contract address hot 交易/selector 分析：已提供只读入口，按合约地址获取 RPC code identity、bounded sampling source、selector/topic candidates、ABI/cache advisory decode、source visibility、partial/rate-limit/proxy/stale/unverified uncertainty states 和 bounded local history hint；它不是交易/审计/risk scoring/全量索引工具，不签名、不广播、不自动保存 analysis 或修改 history。
